package com.app.ricktech.uis.separate_not_book_module.separate_note_book_categories;
...
public class SeparateNoteBookCategoriesActivity extends AppCompatActivity {
                        ...
                        if (response.isSuccessful()) {
                            if (response.body() != null) {
                                if (response.body().getStatus() == 200) {
                                    Intent intent = new Intent(SeparateNoteBookCategoriesActivity.this, GamesActivity.class);
                                    Log.e("size", response.body().getData().size() + "__");
                                    intent.putExtra("data", (Serializable) response.body().getData());
                                    startActivity(intent);

                                } else if (response.body().getStatus() == 403) {
                                    Toast.makeText(SeparateNoteBookCategoriesActivity.this, R.string.no_games, Toast.LENGTH_SHORT).show();
                                }
                            } else {
                                binding.flDialog.setVisibility(View.VISIBLE);
                            }
                    ...
                    public void onResponse(Call<StatusResponse> call, Response<StatusResponse> response) {
                        ...
                        if (response.isSuccessful()) {
                            if (response.body() != null && response.body().getStatus() == 200) {
                                Toast.makeText(SeparateNoteBookCategoriesActivity.this, R.string.suc, Toast.LENGTH_SHORT).show();
                                finish();
                            } else {
                                binding.flDialog.setVisibility(View.VISIBLE);
                            }
    ...
    private void getBuildings() {
        Api.getService(Tags.base_url)
                .getSeparateNoteBookCategoryBuildingByBrandId(lang, brand_id)
    ...
    public void setItemData(int adapterPosition, CategoryModel categoryModel) {
        ...
        if (categoryModel.getIs_final_level().equals("yes")) {
            ...
            req = 200;
            Intent intent = new Intent(this, SeparateNoteBookSubBuildingActivity.class);
