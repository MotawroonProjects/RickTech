package com.app.ricktech.uis.separate_laptop_gaming_module.separate_laptop_gaming_categories;
...
public class SeparateLaptopGamingCategoriesActivity extends AppCompatActivity {
                        ...
                        if (response.isSuccessful()) {
                            if (response.body() != null) {
                                if (response.body().getStatus() == 200) {
                                    Intent intent = new Intent(SeparateLaptopGamingCategoriesActivity.this, GamesActivity.class);
                                    Log.e("size", response.body().getData().size() + "__");
                                    intent.putExtra("data", (Serializable) response.body().getData());
                                    startActivity(intent);

                                } else if (response.body().getStatus() == 403) {
                                    Toast.makeText(SeparateLaptopGamingCategoriesActivity.this, R.string.no_games, Toast.LENGTH_SHORT).show();
                                }
                            } else {
                                binding.flDialog.setVisibility(View.VISIBLE);
                            }
                    ...
                    public void onResponse(Call<StatusResponse> call, Response<StatusResponse> response) {
                        ...
                        if (response.isSuccessful()) {
                            if (response.body() != null && response.body().getStatus() == 200) {
                                Toast.makeText(SeparateLaptopGamingCategoriesActivity.this, R.string.suc, Toast.LENGTH_SHORT).show();
                                finish();
                            } else {
                                binding.flDialog.setVisibility(View.VISIBLE);
                            }
    ...
    private void getBuildings() {
        Api.getService(Tags.base_url)
                .getSeparateLaptopGamingCategoryBuildingByBrandId(lang, brand_id)
    ...
    public void setItemData(int adapterPosition, CategoryModel categoryModel) {
        ...
        if (categoryModel.getIs_final_level().equals("yes")) {
            ...
            req = 200;
            Intent intent = new Intent(this, SeparateLaptopGamingSubBuildingActivity.class);
